...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ─── Stripe Webhook — Map Hack NS automatska obnova ──────────────────────
  //
  // BITNO: Ovaj endpoint mora primiti raw (ne-parsiran) body da bi Stripe
  // signature verifikacija radila. Middleware je registrovan ispod.
  // U Stripe Dashboard → Webhooks, dodati URL: https://cardrop.app/api/stripe/webhook
  // i uključiti događaje: customer.subscription.updated, customer.subscription.deleted, invoice.payment_succeeded
  //
  // Env var STRIPE_WEBHOOK_SECRET mora biti postavljen iz Stripe Webhooks sekcije.
  ...
  app.post('/api/stripe/webhook',
    ...
    async (req: any, res) => {
      ...
      if (!webhookSecret) {
        console.warn('[Stripe Webhook] STRIPE_WEBHOOK_SECRET nije postavljen — webhook nije konfigurisan');
      ...
      try {
        ...
        console.error('[Stripe Webhook] Signature verifikacija neuspešna:', err.message);
      ...
      try {
        if (event.type === 'customer.subscription.updated') {
          ...
          if (!user) {
            console.warn(`[Stripe Webhook] subscription.updated: korisnik sa customerId ${customerId} nije pronađen`);
            return res.json({ received: true });
          }

          if (status === 'active' || status === 'trialing') {
            const expiresAt = new Date(currentPeriodEnd * 1000);
            const plan = sub.metadata?.plan ?? user.mapHackPlan ?? 'premium';
            await storage.renewMapHackPlan(user.id, plan, expiresAt);
            console.log(`[Stripe Webhook] subscription.updated: korisnik ${user.id} — plan ${plan} produžen do ${expiresAt.toISOString()}`);
          } else {
            console.log(`[Stripe Webhook] subscription.updated: status=${status} za korisnika ${user.id} — čekamo deleted event`);
          }
        }
          ...
          const user = await storage.getUserByStripeCustomerId(customerId);
          if (!user) {
            console.warn(`[Stripe Webhook] subscription.deleted: korisnik sa customerId ${customerId} nije pronađen`);
            return res.json({ received: true });
          }

          await storage.deactivateMapHackPlan(user.id);
          console.log(`[Stripe Webhook] subscription.deleted: korisnik ${user.id} deaktiviran → free`);
        }
          ...
          if (invoice.subscription && invoice.billing_reason === 'subscription_cycle') {
            ...
            const user = await storage.getUserByStripeCustomerId(customerId);
            if (user) {
              console.log(`[Stripe Webhook] invoice.payment_succeeded (renewal) za korisnika ${user.id} — subscription.updated će ažurirati expiry`);
            }
          }
        }
          ...
          const user = await storage.getUserByStripeCustomerId(customerId);
          if (user) {
            console.warn(`[Stripe Webhook] invoice.payment_failed za korisnika ${user.id} — Stripe će retry, ne deaktiviramo odmah`);
          }
        }

        res.json({ received: true });
      } catch (err: any) {
        console.error('[Stripe Webhook] Greška pri obradi događaja:', err);
